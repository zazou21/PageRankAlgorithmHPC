...
inline void updatePageRankOMP(const pagerank::CsrGraph& graph,
...
inline void updatePageRankOMPDynamic(const pagerank::CsrGraph& graph,
                                     vector<double>& pagerank,
                                     double damping_factor,
                                     int max_iterations,
                                     double tolerance,
    ...
    for (int iter = 0; iter < max_iterations; ++iter) {
        double dangling_sum = 0.0;
        #pragma omp parallel for reduction(+:dangling_sum) schedule(dynamic, chunk_size)
        for (int node = 0; node < N; ++node) {
...
inline void updatePageRankOMPGuided(const pagerank::CsrGraph& graph,
                                    vector<double>& pagerank,
                                    double damping_factor,
                                    int max_iterations,
                                    double tolerance,
    ...
    for (int iter = 0; iter < max_iterations; ++iter) {
        double dangling_sum = 0.0;
        #pragma omp parallel for reduction(+:dangling_sum) schedule(guided, chunk_size)
        for (int node = 0; node < N; ++node) {
            if (graph.out_degree[node] == 0) dangling_sum += pagerank[node];
        }
        const double base = (1.0 - damping_factor) / N
